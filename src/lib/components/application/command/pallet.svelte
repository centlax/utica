...
<div class="flex flex-col gap-1">
	<div class="relative">
		...
		/>
		<div class="absolute right-2 top-1/2 z-10 -translate-y-1/2 text-sky-900">
			{#if $open}
				<span class="size-4">CU</span>
			{:else}
				<span class="size-4">CD</span>
			{/if}
		</div>
	</div>
</div>
{#if $open}
	...
	>
		...
		>
          ...
          hover:bg-sky-100
          data-[highlighted]:bg-sky-200 data-[highlighted]:text-sky-900
            data-[disabled]:opacity-50"
				>
					{#if $isSelected(manga)}
						<div class="check absolute left-2 top-1/2 z-10 text-sky-900">
							<span class="size-4">CK</span>
						</div>
					{/if}
					<div class="pl-4">
...
<style lang="postcss">
	.check {
		@apply absolute left-2 top-1/2 text-sky-500;
